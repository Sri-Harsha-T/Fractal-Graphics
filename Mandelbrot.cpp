...
inline Mandelbrot::Mandelbrot() {	//constructor function for MAndelbrot class
    DynamicDequeue a;
    Quad_Tree = a;
}

inline bool Mandelbrot::inCardoidOrBulb(double x, double y) {	//Checks whether the point lies inside the Cardoid or the Bulb part of the Mandelbrot set
    double y2 = y * y;
...
inline int Mandelbrot::escape_time(int ix, int iy) {			//Returns the number of iterations before the complex number diverges
...
inline void Mandelbrot::colour(int ix, int iy, int* r1, int* g1, int* b1) {		//assigns rgb values to the colours
...
inline int Mandelbrot::Mandelbrot_Set() {				//implementation of the main futnction
